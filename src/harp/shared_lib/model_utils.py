...
def get_vectorizer(data, vectorizer_path: Optional[str] = None, logger=None):
    """
    ...
    if vectorizer_path is None:
        ...
        vectorizer_path = str(repo_root / 'models' / 'vectorizer.joblib')
    warnings = []
...
def transform_data(vectorizer, data, logger=None):
    ...
    repo_root = Path(__file__).parent.parent.parent
    try:
        joblib.dump(X, str(repo_root / 'models' / 'X.joblib'))
        if logger:
    ...
    try:
        joblib.dump(y, str(repo_root / 'models' / 'y.joblib'))
        if logger:
