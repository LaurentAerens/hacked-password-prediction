...
def get_data(file_path=None, logger=None):
    """
    Load data from a CSV file into a pandas DataFrame and remove all null values.
    
    Parameters:
    file_path (str): The path to the CSV file. Defaults to 'data/combined_data.csv' anchored to project root.
    ...
    if file_path is None:
        # Anchor to project root (4 levels up from shared_lib: shared_lib → harp → src → repo_root)
        file_path = str(Path(__file__).parent.parent.parent.parent / 'data' / 'combined_data.csv')
